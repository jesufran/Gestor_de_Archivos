...
const SyncStatusIndicator: React.FC<{ status: SyncStatus; onRetry: () => void; }> = ({ status, onRetry }) => {
    const statusMap = {
    ...
    const titleText = isError ? `${text} - Clic para reintentar` : text;

    return (
        <button
            onClick={onRetry}
            disabled={!isError}
            className={`flex items-center space-x-2 text-xs font-medium p-1 rounded-md transition-colors ${color} ${
                isError
                ? 'hover:bg-red-100 dark:hover:bg-red-900/50 cursor-pointer'
                : 'cursor-default'
            }`}
            title={titleText}
...
const Header: React.FC<HeaderProps> = ({ title, description, onToggleAiPanel, isAiPanelOpen, globalSearchTerm, onGlobalSearchChange, onSearchFocus, hasPendingTasks, hasInProcessTasks, onBellClick, user, onLogout, syncStatus, onRetrySync }) => {
  return (
